...
function editEvenment2($id, $nom, $description, $photo)
{
try {
$pdo = new PDO('mysql:host=localhost;dbname=ucab_amical', 'root', '');
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
...
$date_Actuelle = date("Y-m-d");
$id_user_connecte = $_SESSION['id_user'];
$requete = $pdo->prepare("UPDATE evenments SET nom=:nom, description=:description,
photo=:photo,
updated_at=:date_Actuelle, updated_by=:created_by WHERE id_evenment=:id_evenment");
$requete->bindParam(':id_evenment', $id);
$requete->bindParam(':nom', $nom);
$requete->bindParam(':description', $description);
$requete->bindParam(':photo', $photo);
